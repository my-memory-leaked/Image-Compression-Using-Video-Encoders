...
def create_square_changing_sequence(image, num_frames, square_size=20):
    height, width, _ = image.shape
    sequence = []
    for i in range(num_frames):
        frame = image.copy()
        x = (i * square_size) % width
        y = ((i * square_size) // width * square_size) % height
        frame[y:y+square_size, x:x+square_size] = [255, 0, 0]  # Zmieniamy kwadrat na czerwony
...
total_length = 25
square_size = 20
sequence = create_square_changing_sequence(image, total_length, square_size)

# Zapisanie sekwencji klatek
folder_name = 'square_changing_sequence'
